    ...
    try{
        document.getElementById(`guild-${i}`).addEventListener("click", function(){
            ...
            if(loggedUser.guild === null){
                ...
                setCookieFromJSON(loggedUser);
                updateData(guilds, users);
                window.location.href = window.location.href.toString().replace("home", "myguild");
            }
            else{
...
if(loggedUser.skillset === null){
    document.getElementById("skillset-window").style.display = "flex";
    loggedUser.skillset = [];
    ...
    function showAvailableSkillset(keyword=""){
        ...
        for(i of availableSkillset){
            let id = "choose-skillset-"+i.replace(" ", "-");
            document.getElementById("skillset").innerHTML += `
        ...
        if(5-loggedUser.skillset.length > 0){
            ...
            for(i of skillset){
                if(document.getElementById("choose-skillset-"+i.replace(" ", "-")) !== null){
                    document.getElementById("choose-skillset-"+i.replace(" ", "-")).addEventListener("click", function(){
                        loggedUser.skillset.push(this.id.toString().replace("choose-skillset-", "").replace("-", " "));
